...
Для подбора оптимальной кнопки предусмотрены:
...
<details>
  ...
  Кнопку можно создать с использованием тега:<br>
    <ul>
        <li><b><</b><b>button></b> с глобальным атрибутом `tabindex="0"` для приёма фокуса клавиатуры;</li>
        <li><b><</b><b>a></b> с атрибутом `href` — `Уточнить детали у разработчика`.</li>
    </ul>
В исключительном случае для кнопки с иконкой или бейджем без текста следует как минимум задать атрибут <b>aria-label</b> с кратким описанием для обработки скринридером — `Уточнить у дизайнера макета и разработчика о такой возможности`.
