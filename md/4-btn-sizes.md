...
Размер задает свойство `size`, например, `size="small"`[^1].<br>
Кнопки позволяет задать пользовательскую высоту в свойстве `vertSize`, например, `vertSize="40"`.<br>
...
:star: Следует огранивайть ширину очень длинных кнопок в больших окнах или помещать их рядом с другими элементами.<br>
...
[^2]: CTA (call to action) — прызыв к действию. Элемент для побуждения пользователя к совершению целевого действия, например: купить товар, подписаться на рассылку.
